...
describe("Auth Utils", () => {
  ...
  describe("saveApiKey", () => {
    it("should save API key to localStorage", () => {
      ...
      expect(localStorage.getItem("python_cloud_server_api_key")).toBe(apiKey);
    });
    ...
    it("should overwrite existing API key", () => {
      ...
      expect(localStorage.getItem("python_cloud_server_api_key")).toBe(
        "new-key"
      );
    });
  });
  ...
  describe("getApiKey", () => {
    it("should return API key from localStorage", () => {
      const apiKey = "test-api-key-123";
      localStorage.setItem("python_cloud_server_api_key", apiKey);
  ...
  describe("removeApiKey", () => {
    it("should remove API key from localStorage", () => {
      ...
      expect(localStorage.getItem("python_cloud_server_api_key")).toBeNull();
    });
  ...
  describe("isAuthenticated", () => {
    it("should return true when API key exists", () => {
      localStorage.setItem("python_cloud_server_api_key", "test-key");
