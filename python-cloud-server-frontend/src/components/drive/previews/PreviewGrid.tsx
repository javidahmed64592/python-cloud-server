...
const extractFolders = (files: FileMetadata[], currentPath: string): FolderItem[] => {
  ...
  files.forEach(file => {
    ...
    const relativePath = currentPath ? file.filepath.slice(pathPrefix.length) : file.filepath;
...
export default function PreviewGrid({ files, onFileClick }: PreviewGridProps) {
  ...
  const filteredFiles = files.filter(file => {
    ...
    return file.filepath.startsWith(currentPath + "/") &&
  ...
  return (
    <>
            ...
            if (item.type === "folder") {
              return (
                <PreviewItem
                  ...
                  filename={item.name}
                  onClick={() => handleFolderClick(currentPath ? `${currentPath}/${item.name}` : item.name)}
                >
              ...
              const filename = item.metadata.filepath.split("/").pop() || item.metadata.filepath;
      ...
      {/* File Display Modals */}
      {selectedFile && displayType === "image" && (
        <ImageDisplay filepath={selectedFile.filepath} onClose={handleCloseDisplay} />
      )}
      {selectedFile && displayType === "text" && (
        <TextDisplay filepath={selectedFile.filepath} onClose={handleCloseDisplay} />
      )}
      {selectedFile && displayType === "video" && (
        <VideoDisplay filepath={selectedFile.filepath} onClose={handleCloseDisplay} />
      )}
      {selectedFile && displayType === "music" && (
        <MusicDisplay filepath={selectedFile.filepath} onClose={handleCloseDisplay} />
      )}
    </>
  );
}
